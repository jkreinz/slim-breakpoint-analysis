...
echo "Each iteration will be saved in simulation_outputs/iteration_X/"
echo ""
...
run_simulation() {
    ...
    mkdir -p simulation_outputs/iteration_$i
    ...
    slim -s $i -d ITER=$i breakpoint_k3.slim > simulation_outputs/iteration_$i/slim_output.log 2>&1
    
    # Check if the simulation completed successfully
    if [ $? -eq 0 ]; then
        echo "Iteration $i completed successfully"
    else
        echo "ERROR: Iteration $i failed! Check simulation_outputs/iteration_$i/slim_output.log"
    fi
}
...
du -sh simulation_outputs/iteration_*
